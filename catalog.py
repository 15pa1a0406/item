...
def main_page():
    ...
    places = session.query(PlaceTitle).order_by(desc(PlaceTitle.id)).limit(10)
...
def show_category(category_id):
    ...
    places = session.query(PlaceTitle).filter_by(category_id=cat.id).order_by(asc(PlaceTitle.name))
    return render_template(
        'category.html',
        categories=categories,
        category=cat,
...
@app.route(
    '/category/<int:category_id>/<int:place_id>/edit/',
    methods=['GET', 'POST']
    )
@login_required
